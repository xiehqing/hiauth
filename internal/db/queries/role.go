...
func (q *Queries) CreateRole(ctx context.Context, role *entity.Role) error {
	return q.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		return q.auditCreate(ctx, tx, "role", role.TableName(), role.ID, role)
	})
}
...
func (q *Queries) DeleteRole(ctx context.Context, id int64) error {
	return q.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var before entity.Role
		if err := tx.Preload("Menus").First(&before, "id = ?", id).Error; err != nil {
			return err
		}
		...
		return q.auditDelete(ctx, tx, "role", before.TableName(), id, before)
	})
}
...
func (q *Queries) UpdateRoleMenus(ctx context.Context, roleID int64, menuIDs []int64) error {
	return q.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		return q.auditAuthorize(ctx, tx, "role_menus", "role_menus", roleID, map[string]any{
			"roleId":  roleID,
...
func replaceRoleMenus(tx *gorm.DB, role *entity.Role, menuIDs []int64) error {
	menus := make([]entity.Menu, 0, len(menuIDs))
	for _, id := range menuIDs {
		if id > 0 {
			menus = append(menus, entity.Menu{BaseModel: ormx.BaseModel{ID: id}})
		}
	}
